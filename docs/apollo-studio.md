---
title: Integrate Tailcall with Apollo Studio for Optimized GraphQL Performance
description: Boost your GraphQL server's performance by integrating Tailcall with Apollo Studio. Our step-by-step guide shows you how to configure Tailcall to send crucial usage metrics to Apollo Studio for advanced monitoring and optimization. Perfect for setting up monolith graphs or enhancing existing ones. Leverage Apollo Studio's powerful features to gain actionable insights and deliver a faster, more efficient GraphQL experience. Get started today and take advantage of Apollo Studio's free trial for new users.
sidebar_label: "Tailcall & Apollo Integration"
---
...
1. Next we need to connect Apollo with our running instance of Tailcall. There are two ways to let Apollo know about your GraphQL schema:
   ...
   1. Navigate to `Local Introspection`. If you have a deployed instance of your GraphQL server you can put the URL pointing to that in `Endpoint URL` and click on `Introspect and Upload`. If not, start a local instance of `tailcall` and put the local url here, similar to how is shown in the image below. You can start a local instance of Tailcall by running `tailcall start` (click [here](/docs/cli) to know more).

      ![local-introspection.png](../static/images/apollo-studio/local-introspection.png)

   1. Or, Navigate to `Local Schema` and insert your schema generated by `tailcall` and click `Upload`. You can get the schema by running `tailcall check` (click [here](/docs/cli) to know more).
...
1. Start `tailcall` with the appropriate configuration for Apollo (click [here](/docs/cli/#start) to know more). Below is an example of what a config may look like:
