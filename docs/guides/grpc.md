...
Also, let's specify options for Tailcall's ingress and egress at the beginning of the config using [`@server`](./directives.md#server) and [`@upstream`](./directives.md#upstream) directives.
...
In our protobuf example file, we have a method called `GetMultipleNews` that we can use. To enable batching we need to enable [`@upstream.batch` option](./directives.md#batch-1) first and specify `batchKey` option for the `@grpc` directive.
