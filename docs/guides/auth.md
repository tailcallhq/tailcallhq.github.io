...
1. With the help of [`@link` directive](./directives.md#link) connect multiple authentication files as you need for different provides. To connect it use either [`Htpasswd`](./directives.md#htpasswd) or [`Jwks`](./directives.md#jwks) link type
2. Mark that some type of field requires authentication to be fetched with the help of [`@protected` directive](./directives.md#protected)
...
To validate authentication for user request the specific headers are used (like `Authorization` header). In case auth is enabled for tailcall those headers will be also added to the [`allowedHeaders` list](./directives.md#allowedheaders) and therefore they will be forwarded to the upstream requests implicitly.
...
To use Basic Auth you should first include htpasswd file generated from [Prerequisites](#prerequisites) with the help of [`@link` directive](./directives.md#htpasswd).
...
After adding `@link` you can use the [`@protected` directive](./directives.md#protected) to mark the fields that requiring success authentication to be requested.
...
To use JWT you should first include JWKS file generated from [Prerequisites](#prerequisites) with the help of [`@link` directive](./directives.md#jwks).
