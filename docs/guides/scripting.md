...
Once you have a worker file ready, you link that file to the tailcall configuration using the [`@link`](./directives.md#link) directive.
...
By default the headers field will be empty in most cases, unless headers are whitelisted via the [allowedHeaders](./directives.md#allowedheaders) setting in [`@upstream`](./directives.md#upstream).
