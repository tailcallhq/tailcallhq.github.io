...
Telemetry configuration is provided by [`@telemetry`](../guides/directives.md#telemetry) directive to setup how and where the telemetry data is send.
...
You can find a reference of type of info generated by Tailcall in the [`@telemetry` reference](../guides/directives.md#telemetry) or consult examples in the next section, in order to gain some understanding.
...
In some cases you may want to customize the data that were added to telemetry payload to have more control over analyzing process. Tailcall supports that customization for specific use cases described below. For eg. the metric [`http.server.request.count`](../guides/directives.md#metrics) can be customized with the [`requestHeaders`](../guides/directives.md#requestheaders) property to allow splitting the overall count by specific headers.
