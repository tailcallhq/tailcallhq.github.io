...
But what exactly is the N +1 problem? It's a situation where the server makes N +1 requests to fetch data for a single GraphQL query. This can lead to inefficient data loading and slow response times.

Understanding and solving this problem is crucial for optimizing your GraphQL queries. It can significantly improve your server's performance and the overall user experience.

In this article, we'll delve into the N +1 problem in detail. We'll explore why it occurs, how it impacts your GraphQL performance, and most importantly, how to solve it with Tailcall's powerful introspection capabilities, how you can identify all instances of N+1 issues even before you start your server, but before we delve deeper into how Tailcall accomplishes this, let's understand the N+1 issue in detail.

## What is the N +1 Problem?

In simple terms, the N +1 problem occurs when your server fetches data in an inefficient manner. Instead of making a single request to retrieve all necessary data, it makes multiple, separate requests.

For instance, if you're fetching a list of users and their posts, an inefficient server might first fetch the users (1 request). Then, for each user, it fetches their posts (N requests). This results in N +1 total requests.

This approach can quickly become problematic as your data grows. It leads to a large number of unnecessary requests, slowing down your server and wasting resources.

## Why is it a Problem for GraphQL?

GraphQL's flexibility is one of its greatest strengths. However, it's also what makes it susceptible to the N +1 problem.

In GraphQL, you can request exactly what you need, down to individual fields. This is great for reducing over-fetching, but it can lead to under-optimized queries.

Each field in a GraphQL query can trigger a resolver function on the server. If these resolvers fetch data independently, they can cause an N +1 problem.
...
:::tip
If you are new here you might want to checkout our [Getting Started](./getting-started.mdx) guide.
:::
...
This configuration sets up a GraphQL schema for a server utilizing [jsonplaceholder.typicode.com](https://jsonplaceholder.typicode.com/) as its data source. It allows direct querying of posts and, for each post, retrieves the associated author information. Similar to our curl requests above, when we query for posts and their authors using graphql query given below we end up issuing multiple user calls upstream:
