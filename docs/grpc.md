...
Also, let's specify options for Tailcall's ingress and egress at the beginning of the config using [`@server`](/docs/guides/directives/#server-directive) and [`@upstream`](/docs/guides/directives/#upstream-directive) directives.
...
In our protobuf example file, we have a method called `GetMultipleNews` that we can use. To enable batching we need to enable [`@upstream.batch` option](/docs/guides/directives/#batch) first and specify `batchKey` option for the `@grpc` directive.
