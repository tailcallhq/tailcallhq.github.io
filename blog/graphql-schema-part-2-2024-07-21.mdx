...
In our [previous post](https://tailcall.run/blog/graphql-schema/),  scalable GraphQL schema is critical for building production-ready APIs that can evolve with your application's needs. In this post, we will dive deeper on how to Continuously evolve your schema to meet your application's changing requirements without Hard versioning.
...
1. **Safe Changes:** These are the golden children of schema evolution. You can make these changes anytime, and your clients won't even bat an eyelash. Adding new fields or types? Go for it!
2. **Dangerous Changes:** These are the sneaky ones. They might not break anything outright, but they can cause subtle issues that'll have your clients scratching their heads. We'll need to tread carefully here.
...
Let's start with the good stuff: additive changes. Most of the time, these are safe as houses. For example, adding fields & adding types is unlikely to cause issues for clients. But, there are a few tricky scenarios to watch out for.

### The Optional Argument Conundrum

Adding optional arguments is generally safe - it's like offering your clients a shiny new toy without forcing them to play with it.However, there's a catch. Check this out:
...
Instead, maintain backwards compatibility by keeping the default behavior the same

### The Required Argument Trap
Now, this is where things get spicy. Adding a required argument is almost always a breaking change. But fear not! There's a way out:
...
This change is breaking, but it doesn't have to be. You can provide a default value for the new argument to keep your existing clients happy.
...
See that `= POPULARITY`? That's your get-out-of-jail-free card. By providing a default value, you've made this addition safe. Existing clients will use the default, and new clients can take advantage of the sorting option.
...
By adding the Organization type, we've expanded what could be returned by queries selecting for `Node`. This could break clients that aren't prepared to handle new types. Always communicate these changes and encourage clients to use proper type checking.
...
Surprise! Your clients might suddenly receive a type they weren't expecting. It's like opening a box of chocolates and finding a pickle - not necessarily bad, but definitely unexpected. Make sure to document how clients should handle these surprise types, and give them a heads-up well in advance.
...
2. **Incorrect Analytics:** If the client tracks search result types, the data will be skewed by the unhandled Comment type.
...
1. Implement a fallback UI component for unknown types:
    ...
    ```
2. Update analytics code to handle unknown types:

    ```javascript
    function trackSearchResult(result) {
      switch (result.__typename) {
        case 'User':
        case 'Post':
        case 'Comment':
          analytics.track(`search_result_${result.__typename.toLowerCase()}`);
          break;
        default:
          analytics.track('search_result_unknown', { type: result.__typename });
      }
    }
    ```
3. Encourage clients to use introspection queries to stay updated on schema changes:
...
Clients that were using exhaustive switches might now have incomplete logic. Always communicate enum changes clearly to your API consumers.
...
:::important
Almost all modifications in place to a schema are **breaking**
:::
...
## Test Your Understanding of Schema Changes
