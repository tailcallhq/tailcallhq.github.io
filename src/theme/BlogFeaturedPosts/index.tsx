        ...
        {items.map((post, index) => (
          ...
          >
            <div className="flex items-center gap-2">
              {post.content.metadata.authors[0] && <BlogAuthor author={post.content.metadata.authors[0]} textClassName="text-content-tiny" />}
            </div>
            <div className="flex flex-col">
              <span className="text-title-tiny text-black">{post.content.metadata.title}</span>
              <span className="text-content-small text-tailCall-light-600 line-clamp-1">{post.content.metadata.description}</span>
            </div>
          </Link>
        ))}
      </div>
